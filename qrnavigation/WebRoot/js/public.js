...
(function($, window, document, undefined) {
    ...
    $.fn.lazyload = function(options) {
        ...
        var settings = {
            ...
            appear          : null,
            load            : null,
            placeholder     : "http://icon.90sheji.com/images/lz.gif"
        };
...
function qqLogin(sign){
    window.open("http://588ku.com/index.php?m=login&a=snsLogin&type=qq&source=" + sign,"","width=750, height=500, top=50, left=50");
}
...
/*公共异步加载*/
$('.action-wrap div,li,ul').on('mouseover',function(event){
    event.stopPropagation();
})
$('.action-wrap').on('mouseover',function(event){
    $.ajax({
        url:"http://ajax.588ku.com/?m=news",
        dataType:'jsonp',
        jsonp:'callback',
        ...
        success:function(data){
            ...
            for(i=0;i<data.length;i++){
                if(data[i]['is_read'] == 1){
                    var li =  '<li class="list newsList"><a href="'+data[i]['url']+'" onclick="newsRead('+data[i]['id']+')" target="_blank" style="text-align:left;width:300px;"><em class="news-circle"></em>'+data[i]['title']+'</a></li>'
                    $('.news-drop-icon').after(li);
                }else{
                    var li =  '<li class="list newsList"><a href="'+data[i]['url']+'" onclick="newsRead('+data[i]['id']+')" target="_blank" style="text-align:left;width:300px;">'+data[i]['title']+'</a></li>'
                    $('.news-drop-icon').after(li);
                }
            }
        }
...
function newsRead(newsId){
    $.ajax({
        url:'http://ajax.588ku.com/?m=ajax&a=newsRead',
        data:{newsId:newsId},
        dataType:'jsonp',
...
$('.action-wrap').mouseleave(function(){
    $('.news-drop').hide();
});
$('.action-wrap div,li,ul').on('mouseover',function(event){
    event.stopPropagation();
...
$(".questionnaire-wd .next-ask").click(function(){
    ...
    $.ajax({
        url:'http://ajax.588ku.com/?m=ajax&a=question',
        dataType:'json',
        xhrFields:{withCredentials:true},
        success:function(data){
        }
...
$(".questionnaire-wd .go-action").click(function(){
    $.ajax({
        url:'http://ajax.588ku.com/?m=ajax&a=questionAdd',
        dataType:'json',
        xhrFields:{withCredentials:true},
        success:function(data){
        }
...
$(document).ready(function(){
    $.ajax({
        url : 'http://588ku.com/?m=PartControl&a=getBarrageData',
        dataType:'json',
        success:function(data){
            if (data.status == 1 && data.data != null) {
                ...
                $("#barrage").find('li').find('.close-btn').click(function () {
                    ...
                    $.ajax({
                        url:'http://588ku.com/?m=index&a=indextest',
                        dataType:'json',
                        success:function(data){}
                    })
                })
...
$(document).ready(function(){
    // $.get('http://ajax.588ku.com/?m=ajaxUserTime&a=ajaxTimer');
    // $.ajax({
    //     url:'http://ajax.588ku.com/?m=ajaxUserTime&a=ajaxTimers',
    //     data:"",
    //     async:true,
    // });
    ...
    // $.getJSON(
    //     "http://ajax.588ku.com/index.php?m=ajax&a=page_cost&exec_time=" + c + "&tp_cost_time=" + tp_cost_time + "&type=" + b + "&callback=?",
    //     function(e) {}
    // );
//  var timestamp = new Date().getTime();
//  var timestamp = parseInt(timestamp/1000);
//  $.post('http://ajax.588ku.com/?m=ajaxUserTime&a=ajaxTimer',{time:timestamp},function(data){
//  });
});
...
$(function(){
    ...
    $('.coll-net-close').bind('click',function(){
        ...
        $.ajax({
            url:'http://ajax.588ku.com/?m=ajax&a=collNetClose',
            dataType:'jsonp',
            xhrFields: {withCredentials: true},
            success:function(data){
            }
    ...
    $('#inspect .sureBtn').on('click',function(){
        ...
        $.ajax({
            url:'http://588ku.com/index.php?m=ajax&a=survey',
            type:'post',
            dataType:'json',
...
$('.dibu a').mouseover(function(){
    $(this).addClass('colo').siblings().removeClass('colo');
    $('.dldd').eq($(this).index()).show().siblings('.dldd').hide();
})
//头部消息中心下拉框
$('.action-wrap').hover(function(){
    $('.action-wrap .info-drop').stop().slideDown(100);
},function(){
    $('.action-wrap .info-drop').stop().slideUp(100);
});

/*广告浏览监控*/
function totalAdIp(webcode,partcode){
    $.getJSON("http://ajax.588ku.com/index.php?m=adIpTotal&webcode="+webcode+"&partcode="+partcode+"&callback=?", function(e) {
    });
}
...
$(function(){
    ...
    $("#topkeyword").keyup(function(J) {
        ...
        if (H != 38 && H != 40 && H != 13) {
            ...
            var L = "http://ajax2.588ku.com/index.php?m=searchtips&a=search&kw=" + K + "&select_index=" + SI + "&callback=?";
...
$(function(){
    ...
    $('#btn-search-back').click(function(){
        ...
        if((index == 0) && (typeof val == 'undefined' || val == null || val == '')){
            window.location.href="http://588ku.com/beijing/";
        }else{
...
//        }
        ...
        if($(this).hasClass("down-big-img")){
            ...
            if(type == 1){
                if(typeof(tab) == "undefined"){
                    if (typeof(pictype) == "undefined") {
                        picDown(rootUrl+"?m=back&a=down&id="+id);
                    } else {
                        ...
                        picDown(rootUrl+"?m=back&a=down&id="+id+"&pictype="+pictype);
                    }
                }else{
                    picDown(rootUrl+"?m=back&a=down&id="+id+"&tab="+tab+"&pro="+pro);//统计多了个参数，区分是成套入口下载的
                }
            }else if(type == 3){
                picDown('/index.php?m=Download&a=peituDownLoad&id=' + id);
            }else{
                if(typeof(tab) == 'undefined'){
                    picDown(rootUrl+"?m=element&a=down&id="+id);
                }else{
                    picDown(rootUrl+"?m=element&a=down&id="+id+"&tab="+tab+"&pro="+pro);//统计多了个参数，区分是成套入口下载的
                }
            }
            return false;
            ...
            if(type == 1){
                if (typeof(pictype) == "undefined") {
                    picDown(rootUrl+"?m=back&a=downpsd&id="+id);
                } else {
                    var cookiePictype = Cookie.getCookie('search_word_to_type');
                    var cookieKeyword = Cookie.getCookie('back_search_keyword');
                    if (cookiePictype) {
                    ...
                    picDown(rootUrl+"?m=back&a=downpsd&id="+id+"&pictype="+pictype);
                }
            }else if(type == 2){
                picDown('/index.php?m=Download&a=downloadPSD&id=' + id + '&picType='+pictype);
            }else if(type == 3){
                picDown('/index.php?m=Download&a=downloadOffice&id=' + id + '&picType='+pictype);
            }else{
                if(typeof(tab) == 'undefined'){
                    picDown(rootUrl+"?m=element&a=downpsd&id="+id);
                }else{
                    picDown(rootUrl+"?m=element&a=downpsd&id="+id+"&tab="+tab);
                }
            }
            return false;
...
function doSearch(val,index){
    ...
    var url = rootUrl+'?m=search&a=pinYin&keyword='+val+'&stype='+index;
    var reurl;

    /*3 banner 4 详情页海报 2 主图 0 背景全部  6 成套  7-> 创意图 8->H5 9-> 店铺  10-> 印刷 5元素   11->模板 12->配图 13->PPT*/

    if(index == 5){
        reurl = 'http://588ku.com/image/';
    }else if(index == 6){
        reurl = 'http://588ku.com/image/';
    }else if(index == 2){
        reurl = 'http://588ku.com/tuku/';
    }else if(index == 12){
        reurl = 'http://588ku.com/pt/';
    }else if(index == 13){
        reurl = 'http://588ku.com/ppt/';
    }else if(index == 14){
        reurl = 'http://588ku.com/resume/';
    }else{
        reurl = 'http://588ku.com/tuku/';
    }
    $.post(url,{},function(data){
        if (data.status == 0) {
            ...
            if(index == 0){
                ...
                reurl = reurl+data.res+'-zhutu.html';
            }else if(index == 4){
                reurl = reurl+data.res+'-xiangqingye.html';
            }else if(index == 5){
                reurl = reurl+data.res+'.html';
            }else if(index == 6){
                reurl = reurl+data.res+'-png.html';
            }else if(index == 8){
                reurl = reurl+data.res+'-h5.html';
            }else if(index == 9){
                reurl = reurl+data.res+'-shop.html';
            }else if(index == 10){
                reurl = reurl+data.res+'-print.html';
            }else if(index == 3){
                reurl = reurl+data.res+'-banner.html';
            }else if(index == 7){
            ...
            // $.get('http://ajax.588ku.com/?m=search&a=recordBGStatisicalData&uid=' + data.uid + '&guid=' + data.guid + '&keyword=' + val + '&pinyin=' + data.res + '&index=' + index);
            $.ajax({
                url:"http://ajax.588ku.com/?m=search&a=recordBGStatisicalData",
                dataType:'jsonp',
...
function picDown(url) {
    $.ajax({url:url,dataType:'json',type:'get',async:false})
        .success(function(data){
            var vipType;
            var vipclass;
            if(data.type == 1){
            ...
            if (data.status == 0) {
                ...
                if (data.no_vip == 1) {
                    setTimeout(function(){
                        $('.model-free-download').closest('.down-limit').show();
                        $('.model-free-download').show();
                    },1000);
                }
                if(data.pic_class == 1){
                    $.ajax({
                        url:"http://ajax.588ku.com/?m=downStatist&a=backDownStatist",
                        dataType:'jsonp',
                    ...
                    $.ajax({
                        url:"http://ajax.588ku.com/?m=downStatist&a=elementDownStatist",
                        dataType:'jsonp',
                    ...
                    $.ajax({
                        url:"http://ajax.588ku.com/?m=downStatist&a=peituDownStatist",
                        dataType:'jsonp',
                ...
                if (vipclass == '.model') {
                    // 模板下载限制提醒次数记录
                    $.get('http://ajax.588ku.com/?m=SponsorFunnel&a=update&mod=1&type=1&uid=' + globaluid);
                }
...
$('.side-barBox .wechat').hover(function(){
    $('.side-barBox .wechat .introduce').append('<img  src="http://js.588ku.com/comp/rightbtn/images/qr-code.png" />');
},function(){
...
//    $.get('http://ajax.588ku.com/?m=SponsorFunnel&a=update&mod=1&type=2&uid=' + globaluid);
//})

// 模板下载限制 企业VIP点击购买提示点击记录
//$('#compay').on('click', function(){
//    if (Urlobj.get('mod') == 1) {
//        $.get('http://ajax.588ku.com/?m=SponsorFunnel&a=update&mod=1&type=4&uid=' + globalUid);
//    }
//});
    ...
    $('body').on('click','.down-big-img,.down-big-y',function(){
        ...
        if(is_com_vip == 1){
            if(type == 1){
                ...
                cate_url = 'http://588ku.com/?m=ProSponsorN&t=2&proType=2&cj=11';
                ...
                cate_url = 'http://588ku.com/?m=ProSponsorN&t=3&proType=2&cj=11';
                ...
                cate_url = 'http://588ku.com/?m=ProSponsorN&proType=2&cj=11';
...
function completeVipHint(cate_type,cate_name,cate_date,cate_url){
    ...
    $.ajax({
        url:'http://ajax.588ku.com/?m=BruthAjax&a=vipTotal',
...
//                      console.log(data);
                ...
                $(function(){
                    $(".search-tab li").on("click",function(){
                        ...
                        if(element){
                            ...
                            $(".search-con .search").removeClass("background model").addClass("element");
                            $(".search-con .search-sug").removeClass("background model").addClass("element");
                            ...
                            $(".search-con .search").removeClass("element model").addClass("background");
                            $(".search-con .search-sug").removeClass("element model").addClass("background");
                            ...
                            $(".search-con .search").removeClass("element background").addClass("model");
                            $(".search-con .search-sug").removeClass("element background").addClass("model");
    ...
    $(function(){
        $(".search-left").hover(function(){
            $(this).children(".current-selectOther").stop().slideToggle('fast');
        })
...
//        })
        ...
        $(".current-selectOther li").click(function(){
            ...
            if ($('#topkeyword_float').attr('data-index') == 0) {
                $('.search-sug-float').removeClass("element model").addClass('background');
                if(modelName == 'BackgroundGallery'){
                    if(isindex == 1){
                ...
                $('.search-sug-float').removeClass("model background").addClass('element');
                if(modelName == 'BackgroundGallery') {
                    if(isindex == 1){
                        $('#topkeyword_float').attr('placeholder', "");
                    } else {
                ...
                $('.search-sug-float').removeClass("element background").addClass('model');
                if(modelName == 'BackgroundGallery') {
                    if(isindex == 1){
                        $('#topkeyword_float').attr('placeholder', "");
                    } else {
            ...
            if(ele){
                $(".search-fixTop .search").removeClass("background model").addClass("element");
                $(".search-fixTop .search-but").removeClass("background model").addClass("element");
                $(".search-fixTop .search-sug").removeClass("background model").addClass("element");
            }else if(bg){
                $(".search-fixTop .search").removeClass("element model").addClass("background");
                $(".search-fixTop .search-but").removeClass("element model").addClass("background");
                $(".search-fixTop .search-sug").removeClass("element model").addClass("background");
            }else if(mod){
                $(".search-fixTop .search").removeClass("element background").addClass("model");
                $(".search-fixTop .search-but").removeClass("element background").addClass("model");
                $(".search-fixTop .search-sug").removeClass("element background").addClass("model");
            }else{
                return;
            }
        })
...
$(function(){
    ...
    $.get('/?m=Popup&a=stUserPopup',function(res){
        if (res.status == 1) {
            var $popup = $('#popup-st-user-popup');
            $popup.find('img').attr('src',res.data.picurl);
            $popup.find('img').attr('data-url',res.data.jumpurl);
            $popup.find('.dl-btn').attr('href',res.data.jumpurl);
